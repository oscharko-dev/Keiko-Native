...
against `main`, and evaluate the Native required-check profile. It may write only checks and
redacted pull-request dashboard comments. It receives no repository contents, Actions,
Administration, or secret permissions.

The aggregate remains advisory. It must not enter `main` protection until negative probes for stale
